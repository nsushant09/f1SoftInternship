...
class DynamicItemUseCase(private val dynamicItemRepo: DynamicItemRepo, private val occupationTypesUseCase: OccupationTypesUseCase) {

    fun getDynamicItemsList() : Observable<DynamicFormResponse> = dynamicItemRepo.getDynamicFormResponse()

    fun getOccupationsList(url : String) : Observable<List<Occupation>> = occupationTypesUseCase.getOccupationsList(url)
